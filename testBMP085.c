...
#define UNIT_SYSTEME_INTERNATIONALE_UNITE
...
// Returns a file id
int bmp085_i2c_Begin(const char *devName){
	int fd;
...
void bmp085_Calibration(const char *i2c_devName){
	int fd = bmp085_i2c_Begin(i2c_devName);
...
unsigned int bmp085_ReadUT(const char *i2c_devName){
	unsigned int ut = 0;
	int fd = bmp085_i2c_Begin(i2c_devName);
...
unsigned int bmp085_ReadUP(const char *i2c_devName){
	unsigned int up = 0;
	int fd = bmp085_i2c_Begin(i2c_devName);
...
void printCommandUsage(FILE *output, const char *prog_name){
	fprintf(output, "Command usage: \n");
	fprintf(output, "%s: <i2c_device_name>, where:\n", prog_name);
	fprintf(output, "i2c_device_name\t\ti2c-dev interface, e.g. /dev/i2c-0\n");
...
int main(int argc, char **argv){
	...
	if (argc < 2){
		fprintf(stderr, "Missing i2c dev parameter\n");
		printCommandUsage(stderr, argv[0]);
		exit(1);
	}
	...
	bmp085_Calibration(i2c_devName);

	temperature = bmp085_GetTemperature(bmp085_ReadUT(i2c_devName));
	pressure = bmp085_GetPressure(bmp085_ReadUP(i2c_devName));
	altitude = bmp085_Altitude(pressure);

#ifndef UNIT_SYSTEME_INTERNATIONALE_UNITE
